                           ...
                           ElevatedButton.icon(
                             ...
                             onPressed: () {
                               ...
                               final nonWinners = docs.where((doc) {
                                 final name = (doc.data() as Map<String, dynamic>)['studentName']?.toString();
                                 return _getRankByName(name) == null;
                               }).toList();
                               
                               if (nonWinners.isEmpty) {
                                 ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("All students are winners. Use Generate Winners Certs.")));
                               } else {
                                 _generateCertificatesFromDocs(nonWinners, isWinner: false);
                               }
                             },
                           ),
                         ],
                                      ...
                                      child: Column(
                                        children: [
                                          ...
                                          Row(children: [
                                            ...
                                            if (rank == null)
                                              Expanded(child: OutlinedButton.icon(icon: const Icon(Icons.card_membership), label: const Text("Cert"), onPressed: () => _generateCertificatesFromDocs([sortedDocs[index]], isWinner: false))),
                                            
                                            // Only show Winner Cert for winners
                                            if (rank != null)
                                              Expanded(child: ElevatedButton.icon(style: ElevatedButton.styleFrom(backgroundColor: Colors.orange), icon: const Icon(Icons.emoji_events), label: const Text("Winner Cert"), onPressed: () => _generateCertificatesFromDocs([sortedDocs[index]], isWinner: true))),
                                          ]),
                                          const SizedBox(height: 8),
                                        ],
                                      ),
        ...
        children: [
          const Padding(
            ...
            child: Text(
              ...
              style: TextStyle(fontSize: 12, color: Colors.blueAccent, fontStyle: FontStyle.italic),
              textAlign: TextAlign.center,
            ),
          ),
          TextField(controller: titleCtrl, decoration: const InputDecoration(labelText: "Main Title"), onChanged: (v) => settings['title'] = v),
          TextField(controller: subtitleCtrl, decoration: const InputDecoration(labelText: "Certify Text"), onChanged: (v) => settings['subtitle'] = v),
          TextField(controller: bodyCtrl, decoration: const InputDecoration(labelText: "Body Template"), maxLines: 2, onChanged: (v) => settings['body'] = v),
          const Divider(),
          Row(children: [
            Expanded(child: TextField(controller: sig1NameCtrl, decoration: const InputDecoration(labelText: "Sig 1 Name"), onChanged: (v) => settings['signatory1Name'] = v)),
            const SizedBox(width: 10),
            Expanded(child: TextField(controller: sig1TitleCtrl, decoration: const InputDecoration(labelText: "Title"), onChanged: (v) => settings['signatory1Title'] = v)),
          ]),
          Row(children: [
            Expanded(child: TextField(controller: sig2NameCtrl, decoration: const InputDecoration(labelText: "Sig 2 Name"), onChanged: (v) => settings['signatory2Name'] = v)),
            const SizedBox(width: 10),
            Expanded(child: TextField(controller: sig2TitleCtrl, decoration: const InputDecoration(labelText: "Title"), onChanged: (v) => settings['signatory2Title'] = v)),
          ]),
          TextField(controller: bgUrlCtrl, decoration: const InputDecoration(labelText: "BG Image URL"), onChanged: (v) => settings['bgUrl'] = v),
          StatefulBuilder(
